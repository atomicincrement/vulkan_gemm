...
fn main() {
    println!("=== Phase 1: GPU Capability Detection ===\n");
    
    match detect_gpu_capabilities() {
        Ok(_) => println!("\n✓ GPU capability detection completed successfully"),
        Err(e) => eprintln!("✗ Error: {}", e),
    }
}

fn detect_gpu_capabilities() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (idx, device) in physical_devices.iter().enumerate() {
        ...
        for (q_idx, queue_family) in queue_families.iter().enumerate() {
            println!("  Family {}: count={}, flags={:?}",
                q_idx, queue_family.queue_count, queue_family.queue_flags);
        }
        
        // Check format support
        println!("\nFormat Support:");
        check_format_support(&instance, *device, "R32G32B32A32_SFLOAT (fp32)", vk::Format::R32G32B32A32_SFLOAT)?;
        check_format_support(&instance, *device, "R16G16B16A16_SFLOAT (fp16)", vk::Format::R16G16B16A16_SFLOAT)?;
        check_format_support(&instance, *device, "R16G16B16A16_SNORM (bfloat16*)", vk::Format::R16G16B16A16_SNORM)?;
...
fn check_format_support(
    ...
    if props.linear_tiling_features.intersects(vk::FormatFeatureFlags::STORAGE_TEXEL_BUFFER) {
        support_types.push("linear");
    }
    if props.optimal_tiling_features.intersects(vk::FormatFeatureFlags::STORAGE_TEXEL_BUFFER) {
        support_types.push("optimal");
    }
    if props.buffer_features.intersects(vk::FormatFeatureFlags::STORAGE_TEXEL_BUFFER) {
